...
class MarketGroup extends React.Component {
    ...
    render() {
        ...
        let marketRows = markets
            .map(market => {
                const name =
                    market.base !== base && market.quote !== base ? (
                        <span>
...
class MyMarkets extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        return (
            ...
            !Immutable.is(
                nextProps.JadexMarkets,
                this.props.JadexMarkets
            ) ||
    ...
    render() {
        ...
        const activeMarketGroup = Object.keys(JadexMarketGroups)[
            activeMarketTab
        ];
        ...
        const isFilteredMarket = m => {
            ...
            if (!!this.state.myMarketFilter) {
                const matchesBase =
                    m.base
                        .replace("JADEX.", "")
                        .indexOf(this.state.myMarketFilter) !== -1;
                const matchesQuote =
                    m.quote
                        .replace("JADEX.", "")
                        .indexOf(this.state.myMarketFilter) !== -1;
        ...
        JadexMarkets &&
            JadexMarkets.filter(isFilteredMarket).map(m => {
                if (JadexMarketGroups[m.base]) {
                    JadexMarketGroups[m.base].push(m);
                }
                if (
                    (!JadexMarketGroups[m.base] &&
                        !JadexMarketGroups[m.quote]) ||
                    (JadexMarketGroups[m.quote] &&
                        !JadexMarketGroups[m.base])
                ) {
                    JadexMarketGroups.OTHER.push(m);
                }
            });

        baseGroups = Object.assign({}, JadexMarketGroups);
            ...
            otherMarkets = activeMarkets
                ...
                .map(market => {
                    ...
                    if (preferredBases.includes(market.base)) {
                        ...
                        // Jadex markets has already been added
...
export default connect(MyMarketsWrapper, {
    listenTo() {
        return [SettingsStore, MarketsStore, AssetStore, JadexStore];
    },
    getProps() {
        return {
