...
class GatewayActions {
    ...
    fetchCoinsSimple({backer = "JADEX", url = undefined} = {}) {
    ...
    fetchPairs() {
        if (!inProgress["fetchTradingPairs"]) {
            ...
            return dispatch => {
                ...
                Promise.all([
                    fetchCoins(
                        JadexAPIs.BASE + JadexAPIs.COINS_LIST
                    ),
                    fetchTradingPairs(
                        JadexAPIs.BASE + JadexAPIs.TRADING_PAIRS
                    ),
                    getActiveWallets(
                        JadexAPIs.BASE + JadexAPIs.ACTIVE_WALLETS
                    )
                ])
                    .then(result => {
                        clearTimeout(fetchCoinsTimeout);
